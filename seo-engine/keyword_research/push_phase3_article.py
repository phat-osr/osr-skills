"""
Generic end-to-end push for any Phase 3 bài.

Reads config from phase3_picks.PICKS[bai_num]. Builds full Articles row,
PATCHes or POSTs to Airtable, uploads serp_insights_json to catbox.

Usage:
    python3 push_phase3_article.py --bai 1 --dry-run
    python3 push_phase3_article.py --bai 1
"""
...
from phase3_picks import PICKS, BATCH_ID, INTERNAL_ANCHORS, OUTBOUND_DEFAULT
...
def build_record(cfg: dict, image_data: dict) -> tuple[dict, list, int]:
    draft_path = Path(__file__).parent / "phase3_drafts" / cfg["draft_file"]
...
def push_serp_insights(cfg: dict, rid: str, dry: bool):
    ...
    payload = {
        ...
        "method": si.get("method", "WebSearch (Claude built-in)"),
...
def main() -> int:
    ...
    ap.add_argument("--bai", type=int, required=True, help="Bài number (1-7)")
    ...
    print(f"=== Phase 3 bài {args.bai} end-to-end ===")
    ...
    if not image_data:
        ...
        image_data = find_cover_image("startup business")
    if not image_data:
        return 1
